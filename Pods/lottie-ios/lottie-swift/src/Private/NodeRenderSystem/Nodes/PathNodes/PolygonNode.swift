...
final class PolygonNodeProperties: NodePropertyMap, KeypathSearchable {
    ...
    init(star: Star) {
        ...
        self.position = NodeProperty(
            provider: KeyframeInterpolator(keyframes: star.position.keyframes))
        ...
        self.rotation = NodeProperty(
            provider: KeyframeInterpolator(keyframes: star.rotation.keyframes))
...
final class PolygonNode: AnimatorNode, PathNode {
    ...
    func rebuildOutputs(frame: CGFloat) {
        ...
        var vertices = [
            CurveVertex(
                point: point + position, inTangentRelative: .zero, outTangentRelative: .zero)
        ]
        ...
        for _ in 0..<Int(ceil(numberOfPoints)) {
            ...
            if outerRoundedness != 0 {
                ...
                vertices[vertices.endIndex - 1] = CurveVertex(
                    previousVertex.inTangent, previousVertex.point,
                    previousVertex.point - cp1 + position)
                vertices.append(
                    CurveVertex(
                        point: point + position, inTangentRelative: cp2, outTangentRelative: .zero))
            } else {
                vertices.append(
                    CurveVertex(
                        point: point + position, inTangentRelative: .zero, outTangentRelative: .zero
