...
final class TextLayer: CALayer {
    ...
    private func drawDebug(_ ctx: CGContext) {
        if let font = font {
            ...
            ctx.fill(
                CGRect(x: 0, y: ascent - capHeight, width: drawingRect.width, height: capHeight))
            ...
            if drawingRect.height - ascent + descent + leading > 0 {
                ...
                ctx.fill(
                    CGRect(
                        x: 0, y: ascent + descent + leading, width: drawingRect.width,
    ...
    override func draw(in ctx: CGContext) {
        ...
        if let setter = fillFrameSetter {
            fillFrame = CTFramesetterCreateFrame(
                setter, CFRangeMake(0, attributedString.length), drawingPath, nil)
        } else {
            fillFrame = nil
        }
        ...
        if let setter = strokeFrameSetter {
            strokeFrame = CTFramesetterCreateFrame(
                setter, CFRangeMake(0, attributedString.length), drawingPath, nil)
        } else {
            strokeFrame = nil
        }
    ...
    private func updateTextContent() {
        ...
        guard let font = font, let text = text, text.count > 0,
            fillColor != nil || strokeColor != nil
        else {
            drawingRect = .zero
            drawingAnchor = .zero
        ...
        if let strokeColor = strokeColor {
            ...
            strokeFrameSetter = CTFramesetterCreateWithAttributedString(
                strokeAttributedString as CFAttributedString)
        } else {
            strokeFrameSetter = nil
            strokeWidth = 0
        }
        ...
        if let preferredSize = preferredSize {
            ...
            drawingRect = CGRect(
                x: 0, y: 0, width: ceil(size.width),
                height: ceil(size.height))
        }

        // Now Calculate Anchor
        drawingAnchor = CGPoint(
            x: textAnchor.x.remap(
                fromLow: 0, fromHigh: drawingRect.size.width, toLow: 0, toHigh: 1),
            y: textAnchor.y.remap(
                fromLow: 0, fromHigh: drawingRect.size.height, toLow: 0, toHigh: 1))
